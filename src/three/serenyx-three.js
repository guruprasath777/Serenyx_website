...
// Read lazily, not at import time: the user can flip the OS setting while the
// tab is open, and an import-time window touch breaks any non-browser import.
...
/* =============================================================================
   ...
   The stage registry is passed in rather than module-global: React may mount
   and unmount this twice (StrictMode does exactly that in dev), and a shared
   array would leak the first mount's renderers into the second one's loop.
============================================================================= */
class Stage {
  constructor(canvas, registry, { fov = 45, position = [0, 0, 7], target = [0, 0, 0] } = {}) {
    this.canvas = canvas;
...
============================================================================= */
function buildHero(canvas, registry) {
  const stage = new Stage(canvas, registry, { fov: 42, position: [0, 0, 8.8] });
  const { scene } = stage;
...
============================================================================= */
function buildPricingGrid(canvas, registry) {
  const stage = new Stage(canvas, registry, { fov: 55, position: [0, 2.4, 7.6], target: [0, -0.4, -3] });
...
/* =============================================================================
   ...
   Called by <ThreeStage> with the canvases React already rendered, so nothing
   here reaches into the document by id. Returns a disposer the effect cleanup
   calls: it cancels the frame loop, drops listeners/observers and frees every
   GPU resource. Browsers cap live WebGL contexts (~16), so skipping this would
   blank the scene after enough remounts.
============================================================================= */
export function createSerenyxScenes({ heroCanvas, pricingCanvas, onHintDismiss } = {}) {
  const noop = { dispose() {}, supported: false };
  ...
  const registry = [];
  if (heroCanvas) buildHero(heroCanvas, registry);
  if (pricingCanvas) buildPricingGrid(pricingCanvas, registry);
  if (!registry.length) return noop;

  let frameId = 0;
  let onPointerOnce = null;

  if (prefersReducedMotion()) {
    // Draw one settled frame and stop: no loop, no hint to chase.
    registry.forEach((st) => { st.inView = true; st.tick(2.4, 1 / 60); });
    if (onHintDismiss) onHintDismiss();
  } else {
    ...
    const loop = () => {
      ...
      last = elapsed;
      for (const st of registry) st.tick(elapsed, delta);
    };
    loop();

    if (onHintDismiss) {
      onPointerOnce = () => onHintDismiss();
      window.addEventListener('pointermove', onPointerOnce, { once: true, passive: true });
    }
  }
  ...
  return {
    ...
    dispose() {
      ...
      if (onPointerOnce) window.removeEventListener('pointermove', onPointerOnce);
      registry.forEach((st) => st.dispose());
      registry.length = 0;
    },
  };
}
